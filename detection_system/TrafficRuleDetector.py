...
def detect(opt):
    source, weights, imgsz = opt["source"], opt["weights"], opt["image_size"]
    ...
    if '.mp4' in source or '.mp3' in source:        
        ...
        while(cap.isOpened()):
            ...
            if counter== framerate:
                ...
                try:
                    ...
                    source = f'main_app/static/{unique_id}Frame.jpg'   
                    cv2.imwrite(source, frame)
                    unique_data["img"] = f"{unique_id}Frame.jpg"
                    print('➡ TrafficRuleDetector.py:79 source:', source)
                    ...
                    for path, img, im0s, vid_cap in dataset:
                        ...
                        for i, det in enumerate(pred):  # detections per image
                                ...
                                for *xyxy, conf, cls in det:
                                    ...
                                    if names[c] == 'Rider':
                                        ...
                                        if rider_helmet_status:
                                            unique_data["helmet_status"] = "Wearing"
                                            print('\n\nRider wearing Helmet')
                                        else:
                                            unique_data["helmet_status"] = "Not Wearing"
                                            print('\n\nRider not wearing Helmet')


                                        if rider_lp_status:
                                            unique_data["lp_status"] = "Found"
                                            print('\nRider having LP')
                                        else:
                                            unique_data["lp_status"] = "Not Found"
                                            print('\nRider not having LP\n\n')

                                        print('\nPlate Number : ',rider_lp_number )
                                        unique_data["plate_number"] = rider_lp_number if rider_lp_number else "-"
                                        print('\nNo. of passengers : ',no_of_passengers )
                                        unique_data["no_of_passengers"] = no_of_passengers
                            ...
                            cv2.imwrite(f'./main_app/static/{unique_id}Frame.jpg', im0)
...
def start_detecttion(file=None):
    ...
    opt["source"] = "dataset/t_image/"
    opt["weights"] = "./runs/train/finalModel/weights/best.pt"
    opt["image_size"] = 448
    opt["device"]="cpu"
...
# start_detecttion(r"G:\4c208feb7a59c96176ee448f863d2ee5_GGoejskr.mp4"))